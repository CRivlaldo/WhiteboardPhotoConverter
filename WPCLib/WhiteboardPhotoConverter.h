...
namespace WPC
{
	...
	private:
		...
		WhiteboardPhotoConverter(const std::string inputFileName);
		WhiteboardPhotoConverter(const std::string inputFileName, Debugger* pDebugger);

		bool Init();
		bool Shutdown();
		bool ConvertToDiagramInternal(Chart*& pOutChart, std::string& errorMessage);
	public:
		static bool ConvertToDiagram(const std::string inputFileName, Chart*& pOutChart, 
